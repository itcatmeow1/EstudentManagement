...
////////////////////////STUDENT////////////////////////////////////
    app.post('/register_student', async (req, res) => {
      ...
      app.post('/login_student', async(req,res)=>{
        ...
        if (!compareid){ //if student_id not same
          ...
          session.student = student_id; // save student id in session
      ...
      app.get('/studentprofile', (req, res) => {
        ...
        studentlist.findOne(session.student).then(results => {
      ...
      app.get('/register_class', (req, res) => {db.collection('class').find().toArray().then(results => {
            studentlist.findOne(session.student).then(resultstudent => {
              console.log(results)
    ...
    app.post('/login_teacher', async(req,res)=>{
      ...
      if (!compareid){ //if teacher_id not same
        ...
        req.session.isAuth = true;
        const session = req.session;
        session.teacher = teacher_id; // save student id in session
    ...
    app.get('/teacherprofile', (req, res) => {
      teacherlist.findOne(session.teacher).then(results => {
    ...
    app.get('/teachedsubject', (req, res) => {
      teacherlist.findOne(session.teacher).then(resultteacher => {
...
app.post('/login_admin', async(req,res)=>{
  ...
  if (!compareid){ //if admin_id not same
    ...
    req.session.isAuth = true;
    const session = req.session;
    session.admin = admin_id; // save student id in session
